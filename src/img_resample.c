...
typedef enum ResizeAbyssPolicy
{
  RESIZE_ABYSS_CLAMP = 0
} ResizeAbyssPolicy;
...
static inline gint
int_floorf (const gfloat x)
{
  return (gint) floorf (x);
}

static inline gint
int_ceilf (const gfloat x)
{
  return (gint) ceilf (x);
}

static inline float
u8_to_float (uint8_t v)
{
...
static float *
resize_sampler_get_ptr (ResizeSampler *self,
                        int            ix,
                        int            iy,
                        int            offset,
...
static void
resize_nohalo_get (      ResizeSampler*    restrict  self,
                         const gdouble                   absolute_x,
                         const gdouble                   absolute_y,
                               ResizeScaleMatrix        *scale,
                               ...
                               ResizeAbyssPolicy           repeat_mode)
{
   ...
   */
  const gfloat* restrict input_ptr =
    (gfloat*) resize_sampler_get_ptr (self, ix_0, iy_0, NOHALO_OFFSET_0,
                                      repeat_mode);
...
static inline void
ewa_update_lohalo (const gint              j,
            const gint              i,
            const gfloat            c_major_x,
            const gfloat            c_major_y,
            const gfloat            c_minor_x,
...
static void
resize_lohalo_get (      ResizeSampler*    restrict  self,
                         const gdouble                   absolute_x,
                         const gdouble                   absolute_y,
                               ResizeScaleMatrix        *scale,
                               void*           restrict  output,
                               ResizeAbyssPolicy           repeat_mode)
{
   ...
   */
  const gfloat* restrict input_ptr =
    (gfloat*) resize_sampler_get_ptr (self, ix_0, iy_0, LOHALO_OFFSET_0,
                                      repeat_mode);
...
static void
resize_nearest_rgba8 (const uint8_t *src,
                      int            src_w,
                      int            src_h,
                      uint8_t       *dst,
                      int            dst_w,
...
static void
resize_bilinear_rgba8 (const uint8_t *src,
                       int            src_w,
                       int            src_h,
                       uint8_t       *dst,
                       int            dst_w,
      ...
      for (int x = 0; x < dst_w; ++x)
        {
          ...
          for (int c = 0; c < 4; ++c)
            {
              float a = u8_to_float (p00[c]) + (u8_to_float (p10[c]) - u8_to_float (p00[c])) * tx;
              float b = u8_to_float (p01[c]) + (u8_to_float (p11[c]) - u8_to_float (p01[c])) * tx;
              float v = a + (b - a) * ty;
              dp[c] = float_to_u8 (v);
            }
        }
...
static void
resize_cubic_rgba8 (const uint8_t *src,
                    int            src_w,
                    int            src_h,
                    uint8_t       *dst,
                    int            dst_w,
...
static void
resize_nohalo_rgba8 (const uint8_t *src,
                     int            src_w,
                     int            src_h,
                     uint8_t       *dst,
                     int            dst_w,
      ...
      for (int x = 0; x < dst_w; ++x)
        {
          double absolute_x = (x + 0.5) * x_ratio;
          float out[4] = {0.f, 0.f, 0.f, 0.f};
          resize_nohalo_get (&sampler, absolute_x, absolute_y, &scale, out, RESIZE_ABYSS_CLAMP);
          uint8_t *dp = dst + (y * dst_w + x) * 4;
          dp[0] = float_to_u8 (out[0]);
          dp[1] = float_to_u8 (out[1]);
          dp[2] = float_to_u8 (out[2]);
...
static void
resize_lohalo_rgba8 (const uint8_t *src,
                     int            src_w,
                     int            src_h,
                     uint8_t       *dst,
                     int            dst_w,
      ...
      for (int x = 0; x < dst_w; ++x)
        {
          double absolute_x = (x + 0.5) * x_ratio;
          float out[4] = {0.f, 0.f, 0.f, 0.f};
          resize_lohalo_get (&sampler, absolute_x, absolute_y, &scale, out, RESIZE_ABYSS_CLAMP);
          uint8_t *dp = dst + (y * dst_w + x) * 4;
          dp[0] = float_to_u8 (out[0]);
          dp[1] = float_to_u8 (out[1]);
          dp[2] = float_to_u8 (out[2]);
...
uint8_t *
ps_img_resample_rgba8 (const uint8_t           *src,
                   int                      src_width,
                   int                      src_height,
                   int                      dst_width,
                   int                      dst_height,
                   PsImgResampleInterpolation  interpolation)
{
  ...
  switch (interpolation)
    ...
    case PS_IMG_RESAMPLE_INTERP_NONE:
      resize_nearest_rgba8 (src, src_width, src_height, dst, dst_width, dst_height);
      break;
    case PS_IMG_RESAMPLE_INTERP_LINEAR:
      resize_bilinear_rgba8 (src, src_width, src_height, dst, dst_width, dst_height);
      break;
